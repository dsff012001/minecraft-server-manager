...
import { agentCommands, auditLog, backups, consoleLogs, lostItems, managedDatabases, mods, nodes, operationLogs, serverPermissions, serverSchedules, servers, user, worlds } from '@/lib/db/schema'
...
export async function GET(request:NextRequest) {
  ...
  if(requestedServerId){
    ...
    const [nodeRows,worldRows,logRows,operationRows,serverPermissionRows,memberRows,scheduleRows,databaseRows]=await Promise.all([
    ...
    return NextResponse.json({
      ...
      schedules:scheduleRows,databases:databaseRows,
...
export async function POST(request:NextRequest) {
  ...
  if(body.action==='create-schedule'){
    ...
    const name=z.string().trim().min(2).max(80).parse(body.name)
    const taskType=z.enum(['restart','backup','log-cleanup']).parse(body.taskType)
